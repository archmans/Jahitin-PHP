...
<div class="pagination">
    <ul>
        <?php if ($activePage > 1) { ?>
            <li>
                <a href="#" onclick="searchTailor(<?php echo $activePage - 1; ?>); return false;">&laquo;</a>
            </li>
        <?php } ?>
        <?php for ($i = $activePage - 1; $i <= $activePage + 1; $i++) { ?>
            <?php if ($i >= 1 && $i <= $pages) { ?>
                <?php if ($i == $activePage) { ?>
                    <li>
                        <a href="#" style="font-weight: bold; color: green;" onclick="searchTailor(<?php echo $i; ?>); return false;"><?= $i; ?></a>
                    </li>
                <?php } else { ?>
                    <li>
                        <a href="#" onclick="searchTailor(<?php echo $i; ?>); return false;"><?= $i; ?></a>
                    </li>
                <?php } ?>
            <?php } ?>
        <?php } ?>
        <?php if ($activePage < $pages) { ?>
            <li>
                <a href="#" onclick="searchTailor(<?php echo $activePage + 1; ?>); return false;">&raquo;</a>
            </li>
        <?php } ?>
    </ul>
</div>
